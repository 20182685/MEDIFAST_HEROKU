...
const ReviewListDESC = (props) => {
  const [comentarios, setComentarios] = useState([]);

  const getComentarios = async () => {
    try {
      const response = await fetch(`http://localhost:5000/comentariosPorMedicoAlta/${props.props.props.id_med}`);
